...
//Analytical formula for the derivative of the arc length of a symmetrical NACA-00xx airfoil
double AnalyticalArcLengthNACA00xx_Derivative(int xx, double x)
{
  double t = (double) xx/100.0;
  double a =  5*t*0.2969;
  double b = -5*t*0.1260;
  ...
  if (std::abs(x) <= 1.0e-8) { x = 1.0e-6; }
  
  double s_prime = 0.0;
  double naca_profile_deriv = 0.5*a/sqrt(x) + b + 2*c*x + 3*d*pow(x,2) + 4*e*pow(x,3);
  s_prime = sqrt(1.0 + pow(naca_profile_deriv,2));
  std::cout << "naca: " << naca_profile_deriv << std::endl;
  std::cout << "s   : " << s_prime << std::endl;
  return s_prime;
}

//Numerical integration to obtain the arc length of a symmetric NACA-00xx airfoil
double AnalyticalArcLengthNACA00xx(int xx, double x)
{
  double t = (double) xx/100.0;
  double a =  5*t*0.2969;
  double b = -5*t*0.1260;
      ...
      for (int i = 0; i < npts; i++)
        {
          xint = (double) i*dx;
          //          std::cout << "dx:    " << dx << std::endl;
          s += AnalyticalArcLengthNACA00xx_Derivative(xx,xint)*dx;
